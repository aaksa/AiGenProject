...
class AIGenProjectSupabaseUser extends BaseAuthUser {
  AIGenProjectSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> aIGenProjectSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AIGenProjectSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
